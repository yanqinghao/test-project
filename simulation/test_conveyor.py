...
def do_move(name, env, out_pipe, msg):
    ...
    yield env.timeout(out_pipe._get_duration())
    out_pipe.push(msg)
...
def message_generator(name, env, out_pipe):
    ...
    for i in range(10):
        ...
        # capAvailable = out_pipe.processing_queue._check_capacity()
        # dist = out_pipe.processing_queue._check_distance()
        if out_pipe._check_available():
            ...
            waiting = max(out_pipe._check_capacity(),
                          out_pipe._check_distance())
            if waiting > 0:
...
def queue_poper(name, env, in_pipe):
    ...
    while True:
        ...
        print(
            f"after pop: {env.now} pop: {msg} queue: {in_pipe.processing_queue}"
        )
...
pipe = Store(env, capacity=10, length=10, speed=1, distance=2)
