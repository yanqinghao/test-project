...
class Store(BaseResource):
    ...
    def __init__(self,
        ...
        self.processing_queue: List[Any] = [None] * length
    ...
    if TYPE_CHECKING:
        ...
        def get(self, item: Any) -> StoreGet:  # type: ignore[override] # noqa: F821
    ...
    def _do_put(self, event: StorePut) -> Optional[bool]:
        if not all(self.processing_queue):
            # self.items.append(event.item)
            maxIdx = max([i for i, data in enumerate(self.processing_queue) if not data])
            self.processing_queue[maxIdx] = {
                "data": event.item,
    ...
    def _do_pop(self, event: StorePop) -> Optional[bool]:
        if self.ready_queue:
            ...
            self.processing_queue = [None] + self.processing_queue[:-1]
        return None
